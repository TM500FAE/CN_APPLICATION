...
# -------------------------------------------------
# This file is not to be released, please mangle
# using perl tidy first to reduce the footprint
# size.  (create_release_perl.bat)
#
# >   perltidy tm500.pl -mangle -dac -o ..\tm500.pl
# -------------------------------------------------
...
sub getDiversifEyeVersion
{
   my $VER_STR = $diversifEye::TestGroup::VERSION;
   $VER_STR =~ s/[^\d.]//g;    # Get just the numeric part.
   @versionParts = split("\\.", $VER_STR);
   return $versionParts[0] . "." . $versionParts[1];
}

##__PACKAGE__->main() unless caller();  # Will use this to allow unit testing

our $VERSION = getDiversifEyeVersion();
...
$DEFINED_NUM_PARTITIONS =~ s/\r//g;
$DEFINED_NUM_PARTITIONS =~ s/\n//g;
$DEFINED_NUM_PARTITIONS =~ s/num_partitions=//g;
...
eval { $xmlConfig->parse($server_target); };
if ($@)
{
...
eval {
   ...
   {
      if (lc($rootKey->{'Use_Reduce'}) eq 'false')
      {
         $ReduceOnOff = "0";
      }
   }
   ...
   {
      if (($rootKey->{'DiversifEye_Type'} eq '500') || ($rootKey->{'DiversifEye_Type'} eq '8400') || ($rootKey->{'DiversifEye_Type'} eq '1000'))
      {
         $DiversifEyeType = $rootKey->{'DiversifEye_Type'};
      }
      elsif (lc($rootKey->{'DiversifEye_Type'}) eq 'teravm')
      {
   ...
   {
      if (($CHASSIS_TYPE eq "1000") && ($DiversifEyeType eq "500"))
      {    #Allow the d1000 to be configured exactly like a d500
   ...
   if (defined $rootKey->{'PPPoE'})
   {
      ...
      {    # Force TeraVM to use adapter default.
         $conf{PPPOE_MAC_START} = "";
      }
      else
      {
   ...
   if (defined $xmlHash->{'diversifEye_Configuration'}->{'Application_Configuration'}->{'Server'})
   {
      ...
      foreach (@{$rootKey})
      {
         ...
         if ($createServerEntry)
         {
            ...
            if ((defined $rootKey->[$i]->{'Physical_Location'}) && (($DiversifEyeType eq "500" || $DiversifEyeType eq "1000")) && ($CORE_EDGE_ENABLED == 1) && ($thisGatewayName eq ""))
            {
               if (lc($rootKey->[$i]->{'Physical_Location'}) eq "core")
               {
                  ...
                  {    # Only create one Core GW
                     ...
                     elsif (defined $rootKey->[$i]->{'diversifEye_Offset'})
                     {
                        if (($rootKey->[$i]->{'diversifEye_Offset'} >= 0) && ($rootKey->[$i]->{'diversifEye_Offset'} <= 99))
                        {
                           $hconf[$j]{Card} = $diversifEyeUser + $rootKey->[$i]->{'diversifEye_Offset'};
                        }
                     }
                     else
                     {
                     ...
                     {
                        if (defined($rootKey->[$i]->{'diversifEye_Port'}))
                        {
                           $hconf[$j]{Port} = $portPrefix . $rootKey->[$i]->{'diversifEye_Port'};
                        }
                        else
                        {
                           $hconf[$j]{Port} = $portPrefix . "1";
                        }
                     }
                     ...
                     $hconf[$j]{Location}     = "Core";
                     $hconf[$j]{ServerIp}     = "";
                     $hconf[$j]{ServerMask}   = $v6_MaskBits;
                     $hconf[$j]{PooledPrefix} = $PooledSgiPrefix;
                     $hconf[$j]{PooledUnit}   = "1";
                     if ($VERSION >= 12.0)
                     {
            ...
            if (($thisServerType eq "External") && ($rootKey->[$i]->{'Application'} ne "PING"))
            {    # PING can only be internal.
            ...
            elsif ($thisServerType eq "Internal")
            {
               ...
               if (defined $rootKey->[$i]->{'Mask_Bits'})
               {
                  if ($rootKey->[$i]->{'Mask_Bits'} ne "")
                  {
                     if ($thisIpVersion eq "6")
                     {
                        if (($rootKey->[$i]->{'Mask_Bits'} >= 1) && ($rootKey->[$i]->{'Mask_Bits'} <= 65536))
                        {
                           $hconf[$j]{ServerMask} = $rootKey->[$i]->{'Mask_Bits'};
                        }
                     }
                     else
                     {
                        if (($rootKey->[$i]->{'Mask_Bits'} >= 1) && ($rootKey->[$i]->{'Mask_Bits'} <= 30))
                        {
                           $hconf[$j]{ServerMask} = $rootKey->[$i]->{'Mask_Bits'};
                        }
                     }
                        ...
                        foreach (@{$thisKey})
                        {
                           if (defined $thisKey->[$thisKeyIndex]->{'SIP_Server'})
                           {
                              if (defined $thisKey->[$thisKeyIndex]->{'SIP_Server'}->{'Server_Host_Name'})
                              {
                                 ...
                                 if (($thisKey->[$thisKeyIndex]->{'SIP_Server'}->{'Server_Host_Name'} eq $hconf[$j]{HostName}) && ($doPerPortVoIPProxy == 1))
                                 {
...
if (($FtpPutEnabled == 1) && ($FtpServerEnabled == 0))
{
   $FtpPutEnabled = 0;
}

if (($FtpGetEnabled == 1) && ($FtpServerEnabled == 0))
{
   $FtpGetEnabled = 0;
}

if (($HttpEnabled == 1) && ($HttpServerEnabled == 0))
{
   $HttpEnabled = 0;
}

if (($VoipEnabled == 1) && ($VoipServerEnabled == 0))
{
   $VoipEnabled = 0;
}

if (($RtspEnabled == 1) && ($RtspServerEnabled == 0))
{
   $RtspEnabled = 0;
}

if (($TwampEnabled == 1) && ($TwampServerEnabled == 0))
{
   $TwampEnabled = 0;
}

if (($SpingEnabled == 1) && ($PingServerEnabled == 0))
{
   $SpingEnabled = 0;
}

if (($TeraFlowEnabled == 1) && ($TeraFlowServerEnabled == 0))
{
   $TeraFlowEnabled = 0;
}

if (($IgmpEnabled == 1) && ($IgmpServerEnabled == 0))
{
   $IgmpEnabled = 0;
}

if (($MldEnabled == 1) && ($MldServerEnabled == 0))
{
   $MldEnabled = 0;
}
...
for ($profileId = -1 ; $profileId <= 9 ; $profileId++)
{
   ...
   if (defined $ClientProfilesKey->{$profileName})
   {
      ...
      if (((defined $loadProfilesKey->{'VoIP'}) || (defined $loadProfilesKey->{'VoIMS'})) && ($VoipEnabled == 1))
      {
         $thisKey   = ();
         $nextKeyId = 0;
...
for ($profileId = -1 ; $profileId <= 9 ; $profileId++)
{
   ...
   if (defined $ClientProfilesKey->{$profileName})
   {
      ...
      if ((defined $loadProfilesKey->{'FTP_Get'}) && ($FtpGetEnabled == 1))
      {
         $thisKey = ();
      ...
      if ((defined $loadProfilesKey->{'FTP_Put'}) && ($FtpPutEnabled == 1))
      {
         $thisKey = ();
      ...
      if ((defined $loadProfilesKey->{'HTTP'}) && ($HttpEnabled == 1))
      {
         $thisKey = ();
         $thisKey = $xmlHash->{'diversifEye_Configuration'}->{'Application_Configuration'}->{'Client_Profiles'}->{$profileName}->{'HTTP'};
         $i       = 0;
      ...
      if ((defined $loadProfilesKey->{'SHTTP'}) && ($HttpEnabled == 1))
      {
         $thisKey = ();
      ...
      if ((defined $loadProfilesKey->{'TWAMP'}) && ($TwampEnabled == 1))
      {
         $thisKey = ();
      ...
      if ((defined $loadProfilesKey->{'TeraFlow'}) && ($TeraFlowEnabled == 1))
      {
         $thisKey = ();
         $thisKey = $xmlHash->{'diversifEye_Configuration'}->{'Application_Configuration'}->{'Client_Profiles'}->{$profileName}->{'TeraFlow'};
         $i       = 0;
      ...
      if ((defined $loadProfilesKey->{'RTSP'}) && ($RtspEnabled == 1))
      {
         $thisKey = ();
         $thisKey = $xmlHash->{'diversifEye_Configuration'}->{'Application_Configuration'}->{'Client_Profiles'}->{$profileName}->{'RTSP'};
         $i       = 0;
            ...
            foreach (@{$thisKey})
            {
               ...
               if ((grep /^$AliasEntryName/, @RtspAliasNames) == 0)
               {
                  ...
                  if ((defined $thisKey->[$i]->{'Server_Host_Name'}) || (defined $thisKey->[$i]->{'Path'}))
                  {
                     ...
                     if ((grep /^$RtspPath/, @RtspResourcePaths) == 0)
                     {
                        push(@RtspResourcePaths, $RtspPath);
                     ...
                     else
                     {
                        for $ue (0 .. $conf{UEs} - 1)
                        {
                           ...
                           if ($createEntry)
                           {
                              ...
                              if ((grep /^$host_name/, @VoIPApps) == 0)
                              {    # only one VoIP type client per UE host
...
for $i (0 .. $#hconf)
{
   ...
   $D6o->Get('option_request')->Set('requested_option_codes', '1,6,22');
   $D6c = diversifEye::Dhcp6Config->new(dhcp_options => $D6o);

   ### Global PPPoE Configuration ###
   my $Px = diversifEye::PppoeSettings->new(
                                            access_concentrator              => '',
                                            service_name                     => 'TestUk3',
                                            ...
                                            is_magic_number_used             => 'false',
                                            is_double_retransmit_time        => 'false',
                                           );
   ...
   {
      $Tg->Add(
               diversifEye::ExternalHost->new(
                                              name       => $hconf[$i]{HostName},
   ...
   if (($hconf[$i]{Type} eq 'v4_DVHS') || ($hconf[$i]{Type} eq 'v6_DVHS'))
   {
      ...
      if ($hconf[$i]{Application} eq 'PING')
      {
         if ($hconf[$i]{ServerIp} ne "")
         {
            ...
            {
               $thisIpAddress = diversifEye::IpAddress->new("$hconf[$i]{ServerIp}" . "/" . "$hconf[$i]{ServerMask}");
            }
            else
            {
               $thisIpAddress = diversifEye::IpAddress->new("$hconf[$i]{ServerIp}" . "/" . "$hconf[$i]{ServerMask}");
      ...
      if ((grep /^$thisHostName/, @InternalHostNames) == 0)
      {
         ...
         if (($useScaledEntities == 1) && ($hconf[$i]{Application} eq 'PING'))
         {
            #          $Tg->NewTemplate();
         ...
         else
         {
            $Tg->NewTemplate();
            $Tg->Add(
                     diversifEye::DirectVirtualHost->new(
                                                         name                        => $thisHostName,
                                                         ip_address                  => $thisIpAddress,
   ...
   if ($hconf[$i]{Type} eq 'v4_DVH')
   {
      ...
      if ($conf{addr_assignment} eq 'DHCP')
      {
         ...
         for $x (1 .. $hconf[$i]{Number})
         {
            $Tg->Add(
                     diversifEye::DirectVirtualHost->new(
                                                         name                        => $hconf[$i]{HostName} . "_RG" . sprintf("%05s", $x),
      ...
      elsif ($conf{addr_assignment} eq 'PPPoE')
      {
         ...
         for $x (1 .. $hconf[$i]{Number})
         {
            $Ps = diversifEye::PppoeSettings->new(
                                                  access_concentrator              => '',
                                                  service_name                     => $hconf[$i]{Outer},
                                                  is_checking_host_uniq            => 'false',
                                                  is_pap_authentication_supported  => $conf{PPPOE_PAP},
                                                  is_chap_authentication_supported => $conf{PPPOE_CHAP},
                                                  is_magic_number_used             => 'false',
   ...
   elsif ($hconf[$i]{Type} eq 'CSH')
   {
      $Tg->NewTemplate();
      $Tg->Add(
               diversifEye::DirectVirtualSubnetHost->new(
                                                         name                        => $hconf[$i]{HostName},
...
if ($DiversifEyeType eq "1000")
{
   ...
   {    # Create the defaults for the d1000
      $halfUEs                              = floor(($conf{UEs} - 1) / 2);
      $thisPPPoECard                        = $conf{PPPOE_CARD};
      $tmpmKey                              = ();
...
if ($useScaledEntities == 1)
{
   ...
   for $pdn (0 .. $PDNs_per_UE - 1)
   {
      # IPv4 Hosts
      $Ps = diversifEye::PppoeSettings->new(
                                            service_name                     => diversifEye::PsAlnum->new(prefix_label => $service_name_prefix, suffix_label => "_" . $pdn, starting_at => 0, increment_size => 1),
                                            is_pap_authentication_supported  => $conf{PPPOE_PAP},
                                            is_chap_authentication_supported => $conf{PPPOE_CHAP}
                                           );

      $Ps6 = diversifEye::PppoeSettings->new(
                                             service_name                         => diversifEye::PsAlnum->new(prefix_label => $service_name_prefix, suffix_label => "_" . $pdn, starting_at => 0, increment_size => 1),
                                             is_pap_authentication_supported      => $conf{PPPOE_PAP},
                                             is_chap_authentication_supported     => $conf{PPPOE_CHAP},
         ...
         foreach (@{$tmpmKey})
         {
            ...
            if ($createPdnEntry == 1)
            {
               ...
               foreach $thisUeRange (@UeGroups)
               {
                  ...
                  $Ps = diversifEye::PppoeSettings->new(
                                                        service_name                     => diversifEye::PsAlnum->new(prefix_label => $service_name_prefix, suffix_label => "_" . $pdn, starting_at => $startingAt, increment_size => $incrementSize),
                                                        is_pap_authentication_supported  => $conf{PPPOE_PAP},
                                                        is_chap_authentication_supported => $conf{PPPOE_CHAP}
                                                       );
   ...
   for $pdn (0 .. $PDNs_per_UE - 1)
   {
      ...
      else
      {
         for $ue (0 .. $conf{UEs} - 1)
         {
            ...
            elsif (($isIPv6Host == 1) && (index($bPingTargetIp, ":") != -1))
            {
               $Tg->NewTemplate();
               $Tg->Add(
                        diversifEye::PingApp->new(
...
for $i (0 .. $#hconf)
{
   ...
   if ($hconf[$i]{Type} eq 'v4_DVHS' && $hconf[$i]{Application} eq 'HTTPServer')
   {
      ...
      if (($VERSION >= 8) && ($HttpTlsSupportNeeded == 1))
      {    # Add HTTPS (TLS) Server also
         $Tg->NewTemplate();
         $Tg->Add(
                  diversifEye::HttpServer->new(
   ...
   if ($hconf[$i]{Type} eq 'v4_EServer' && $hconf[$i]{Application} eq 'HTTPServer')
   {
      $Tg->NewTemplate();
      $Tg->Add(
               diversifEye::ExternalHttpServer->new(
                                                    name                 => $hconf[$i]{HostName} . "_http",
                                                    host                 => $thisHostName,
                                                    administrative_state => $AsOther
                                                   )
   ...
   if ($hconf[$i]{Type} eq 'v4_EServer' && $hconf[$i]{Application} eq 'FTPServer')
   {
      $Tg->NewTemplate();
      $Tg->Add(
               diversifEye::ExternalFtpServer->new(
                                                   name                 => $hconf[$i]{HostName} . "_ftp",
                                                   host                 => $thisHostName,
                                                   administrative_state => $AsOther
                                                  )
   ...
   if ($hconf[$i]{Application} eq 'ExternalSIPServer')
   {
      ...
      if (index($hconf[$i]{ServerPorts}, ",") != -1)
      {
         ...
         foreach $portVal (@sipPorts)
         {
            ...
            $Tg->NewTemplate();
            $Tg->Add(
                     diversifEye::ExternalVoipSipProxy->new(
                                                            name                 => $thisSipValName,
                                                            host                 => $thisHostName,
   ...
   if ($hconf[$i]{Type} eq 'v4_EServer' && $hconf[$i]{Application} eq 'RTSPServer')
   {
      ...
      $Tg->NewTemplate();
      $Tg->Add(
               diversifEye::ExternalRtspServer->new(
                                                    name                 => $hconf[$i]{HostName} . "_rtsp",
                                                    host                 => $thisHostName,
   ...
   if ($hconf[$i]{Type} eq 'v4_EServer' && $hconf[$i]{Application} eq 'TWAMPServer')
   {
      $Tg->NewTemplate();
      $Tg->Add(
               diversifEye::ExternalTwampServer->new(
                                                     name                 => $hconf[$i]{HostName} . "_twamp",
                                                     host                 => $thisHostName,
                                                     administrative_state => $AsOther
                                                    )
   ...
   if ($VERSION >= 10)
   {    # Only available on v10 and above
      if ($hconf[$i]{Type} eq 'v4_DVHS' && $hconf[$i]{Application} eq 'TeraFlowServer')
      {
         ...
         $Tg->NewTemplate();
         $Tg->Add(
                  diversifEye::TeraFlowServer->new(
      ...
      if ($hconf[$i]{Type} eq 'v4_EServer' && $hconf[$i]{Application} eq 'TeraFlowServer')
      {
         $Tg->NewTemplate();
         $Tg->Add(
                  diversifEye::ExternalTeraFlowServer->new(
                                                           name                 => $hconf[$i]{HostName} . "_tf",
                                                           host                 => $thisHostName,
                                                           transport_port       => $hconf[$i]{TransportPort},
                                                           administrative_state => $AsOther
   ...
   if ($VERSION >= 11)
   {    # Only available on v11 and above
      if ($hconf[$i]{Type} eq 'v6_DVHS' && $hconf[$i]{Application} eq 'MLDServer')
      {
      ...
      if ($hconf[$i]{Type} eq 'v6_EServer' && $hconf[$i]{Application} eq 'MLDServer')
      {
         ...
         $Tg->NewTemplate();
         $Tg->Add(
                  diversifEye::ExternalMldServer->new(
                                                      name                 => $thisServerName,
                                                      host                 => $thisHostName,
      ...
      if ($hconf[$i]{Type} eq 'v4_EServer' && $hconf[$i]{Application} eq 'IGMPServer')
      {
         ...
         $Tg->NewTemplate();
         $Tg->Add(
                  diversifEye::ExternalIgmpServer->new(
                                                       name                 => $thisServerName,
                                                       host                 => $thisHostName,
                                                       multicast_group_list => "Iggl" . $hconf[$i]{HostName} . "_igmp",
                                                       administrative_state => $AsOther
...
for ($profileId = -1 ; $profileId <= 9 ; $profileId++)
{
   ...
   if ((defined $loadProfilesKey->{'SHTTP'}) && ($HttpEnabled == 1))
   {    # Don't create if there are no http clients
      $thisKey = ();
      $thisKey = $xmlHash->{'diversifEye_Configuration'}->{'Application_Configuration'}->{'Client_Profiles'}->{$profileName}->{'SHTTP'};
      $i       = 0;
               ...
               else
               {
                  ...
                  for $ue (0 .. $conf{UEs} - 1)
                  {
                     ...
                     if (defined $thisKey->[$i]->{'UE_Pattern'})
                     {
                        ...
                        {
                           if ($ue % 2)
                           {    # If the UE is odd then clear the create flag.
                              $createEntry = 0;
                           }
                        }
                        ...
                        {
                           if ($ue % 2 == 0)
                           {    # If the UE is even then clear the create flag.
                              $createEntry = 0;
                           }
                        }
                     }
...
for ($profileId = -1 ; $profileId <= 9 ; $profileId++)
{
   ...
   if ((defined $loadProfilesKey->{'TeraFlow'}) && ($TeraFlowEnabled == 1) && ($VERSION >= 10))
   {
      $thisKey = ();
      $thisKey = $xmlHash->{'diversifEye_Configuration'}->{'Application_Configuration'}->{'Client_Profiles'}->{$profileName}->{'TeraFlow'};
      $i       = 0;
...
for ($profileId = -1 ; $profileId <= 9 ; $profileId++)
{
                  ...
                  if ($useScaledEntities == 1)
                  {
                     ...
                     if ($createPdnEntry == 1)
                     {
                        ...
                        foreach $thisUeRange (@UeGroups)
                        {
                           ...
                           {
                              $Tg->Add(
                                       diversifEye::HttpAdaptiveClient->new(
                                                                            scale_factor                            => $scaleFactor,
                                                                            name                                    => diversifEye::PsAlnum->new(prefix_label => $prefixLabel, suffix_label => $suffixLabel, starting_at => $startingAt, increment_size => $incrementSize, padding_enabled => $useScaledPadding, value_override => $Alias . "_" . $overrideName),
   ...
   if ((defined $loadProfilesKey->{'RTSP'}) && ($RtspEnabled == 1))
   {
      ...
      foreach (@{$thisKey})
      {
         ...
         if ((defined $thisKey->[$i]->{'Server_Host_Name'}) || (defined $thisKey->[$i]->{'Path'}))
         {
            ...
            {
               $mediaPorts = '';
               if ($thisRtspMediaTransport eq 'MPEG2-TS')
               {    # This is also only allowed in TCP mode.
                  $thisRtspMediaTransport = 'MPEG2-TS/RTP';
               }
            }
            else
                  ...
                  if ($useScaledEntities == 1)
                  {
                     ...
                     if ($createPdnEntry == 1)
                     {
                        ...
                        foreach $thisUeRange (@UeGroups)
                        {
                           ...
                           $Tg->Add(
                                    diversifEye::TeraFlowClient->new(
                                                                     scale_factor             => $scaleFactor,
                                                                     name                     => diversifEye::PsAlnum->new(prefix_label => $Alias . "_", suffix_label => "_" . $pdn, starting_at => $startingAt, increment_size => $incrementSize, padding_enabled => $useScaledPadding, value_override => $Alias . "_" . $overrideName),
   ...
   #   MLD Client
   #
   if ((defined $loadProfilesKey->{'MLD'}) && ($MldEnabled == 1) && ($VERSION >= 11))
   {
   ...
   #   IGMP Client
   #
   if ((defined $loadProfilesKey->{'IGMP'}) && ($IgmpEnabled == 1) && ($VERSION >= 11))
   {
   ...
   #   cPing Client
   #
   if ((defined $loadProfilesKey->{'cPing'}) && ($CpingEnabled == 1) && ($VERSION >= 8.5))
   {
               ...
               else
               {
                  ...
                  for $ue (0 .. $conf{UEs} - 1)
                  {
                     ...
                     if ($createEntry)
                     {
                        ...
                        elsif (($isIPv6Host == 1) && (index($thisPingTargetIp, ":") != -1))
                        {
                           $Tg->NewTemplate();
                           $Tg->Add(
                                    diversifEye::PingApp->new(
   ...
   #   sPing Client
   #
   if ((defined $loadProfilesKey->{'sPing'}) && ($SpingEnabled == 1) && ($VERSION >= 8.5))
   {
      ...
      foreach (@{$thisKey})
      {
         ...
         for $pdn (0 .. $PDNs_per_UE - 1)
         {
            ...
            {
               $AliasEntryName = $Alias . "_" . $rangeStr . "_" . $profileName . "_PDN" . $pdn;    # The default name
            }
            else
            {
               $AliasEntryName = $Alias . "_" . $rangeStr . "_" . $profileName . "_SPING_PDN" . $pdn;
            }
      ...
      foreach (@{$thisKey})
      {
         ...
         if (defined $thisKey->[$i]->{'SIP_Server'}->{'Server_Host_Name'})
         {
            ...
            if (defined $thisKey->[$i]->{'VoLTE'}->{'AKA_Key'})
            {
               if ($thisKey->[$i]->{'VoLTE'}->{'AKA_Key'} ne "")
               {
                  ...
                  if ($thisTempAkaKey =~ /^[0-9a-fA-F]+$/)
                  {
                     ...
                     if (($useScaledEntities == 1) && ($thisVoLteAkaKeyUeId == 1))
                     {
                        $thisVoLteAkaKey = diversifEye::PsHex->new(start => $thisTempAkaKey, increment_size => $thisVoLteAkaKeyUeId);
                     }
                     elsif (($useScaledEntities == 1) && ($thisVoLteAkaKeyUeId == 0))
                     {
                        $thisVoLteAkaKey = $thisTempAkaKey;
                     }
                     else
                     {
            ...
            for $pdn (0 .. $PDNs_per_UE - 1)
            {
               ...
               $ipVersionEntryStr = "";
               if (((grep /^$thisSIPServer/, @IpV6ServerHostNames) != 0) && ($useScaledEntities == 1))
               {
                  ...
                  if ($useScaledEntities == 1)
                  {
                     ...
                     if ($createPdnEntry == 1)
                     {
                        ...
                        foreach $thisUeRange (@UeGroups)
                        {
                           ...
                           if (($SIPTransportPort != $thisSIPPort) && ($doPerPortVoIPProxy == 1))
                           {
                              $thisSIPServerName = $thisSIPServer . "_" . $thisSIPPort;
                           }

                           if ((grep /^$thisSIPServerName/, @InternalVoIPServerNames) != 0)
                           {    # If we are to connect to a diversifEye internal server
                              $thisSIPRegisterWithServer = "false";
                           }
                     ...
                     for $ue (0 .. $conf{UEs} - 1)
                     {
                        ...
                        if ($createEntry)
                        {
                           ...
                           if (defined $thisKey->[$i]->{'Mobile_Originated_Pattern'})
                           {
                              ...
                              if ((grep /^$thisSIPServer/, @InternalVoIPServerNames) != 0)
                              {    # If we are to connect to a diversifEye internal server
                                 $thisSIPRegisterWithServer = "false";
                              }
                              ...
                              if ((grep /^$thisVoIPAppID/, @VoIPApps) == 0)
                              {    # only one VoIP type client per UE host
                                 ...
                                 {    # No VoLTE support in this version.
                                 ...
                                 {    # API changed and broken by Shenick.
                                    $Tg->Add(
                                             diversifEye::VoipUa->new(
                                                                      name                               => $Alias . "_" . $base_name,
                                                                      description                        => $thisVoipDescription,
...
if (($VERSION >= 11.3) && ($PooledConfig == 0))
{    # Not yet supported for Pool Manager Mode.
   $rootKey = $xmlHash->{'diversifEye_Configuration'};
   if (defined $rootKey->{'Global_Settings'})
   {
      ...
      if (defined $thisKey->{'Wireless_call_answering_with_183_180_responses'})
      {
         ...
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "3GPP Wireless call answering with 183,180 responses");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Send_early_media_if_183_session_progress_received'})
      {
         ...
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Send early media if 183 Session Progess received");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Route_SIP_responses_using_VIA_field'})
      {
         ...
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Route SIP responses using the VIA field of request");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Override_VolTE_AKA_ESP_to_use_null_encryption'})
      {
         if (lc($thisKey->{'Override_VolTE_AKA_ESP_to_use_null_encryption'}) eq "true")
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Override VolTE-AKA-ESP to use null encryption");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'VoIP_Immediate_stop_when_stopping'})
      {
         if (lc($thisKey->{'VoIP_Immediate_stop_when_stopping'}) eq "true")
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Immediate stop when stopping");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'VoIP_Registration_must_succeed_before_calls_start'})
      {
         if (lc($thisKey->{'VoIP_Registration_must_succeed_before_calls_start'}) eq "true")
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Registration must succeed before calls start");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Reuse_TCP_connection_for_SIP_sessions'})
      {
         if (lc($thisKey->{'Reuse_TCP_connection_for_SIP_sessions'}) eq "true")
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Enable re-use TCP connection policy for SIP sessions");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Enable_precondition_support_in_SIP_messages'})
      {
         if (lc($thisKey->{'Enable_precondition_support_in_SIP_messages'}) eq "true")
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Enable precondition support in SIP messages");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      # Provisional response code 101-199 sent in response to INVITE [-1 for none]:
      if (defined $thisKey->{'Provisional_response_code_sent_in_response_to_INVITE'})
      {
         if ($thisKey->{'Provisional_response_code_sent_in_response_to_INVITE'} ne "")
         {
            $paramName = getGlobalParamName("VoIP Client Settings", "Provisional response code 101-199 sent in response to INVITE");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
               ...
               $postData .= $paramName . "=" . $thisKey->{'Provisional_response_code_sent_in_response_to_INVITE'};
            }
         }
      }
      ...
      if (defined $thisKey->{'SIP_Enable_PANI_headers'})
      {
         ...
         {
            $paramName = getGlobalParamName("SIP Field Settings", "Enable PANI Headers");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      {
         $paramName = getGlobalParamName("SIP Field Settings", "Access Type");
         if ($paramName ne "")
         {
      ...
      {
         $paramName = getGlobalParamName("SIP Field Settings", "Access Info");
         if ($paramName ne "")
         {
      ...
      if (defined $thisKey->{'SIP_Access_Value'})
      {
         ...
         {
            $paramName = getGlobalParamName("SIP Field Settings", "Access Value");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'SIP_Number_of_Response_Fields'})
      {
         ...
         {
            $paramName = getGlobalParamName("SIP Field Settings", "Number of SIP Response Fields");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Enable_IPv4_fragment_reassembly'})
      {
         if (lc($thisKey->{'Enable_IPv4_fragment_reassembly'}) eq "true")
         {
            $paramName = getGlobalParamName("Host IPv4 Settings", "Enable IPv4 fragment reassembly");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Enable_IPv6_fragment_reassembly'})
      {
         if (lc($thisKey->{'Enable_IPv6_fragment_reassembly'}) eq "true")
         {
            $paramName = getGlobalParamName("Host IPv6 Settings", "Enable IPv6 fragment reassembly");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Attempt_DHCPv6_IPv6_Address_Auto_Config_fails'})
      {
         if (lc($thisKey->{'Attempt_DHCPv6_IPv6_Address_Auto_Config_fails'}) eq "true")
         {
            $paramName = getGlobalParamName("Host IPv6 Settings", "Attempt DHCPv6 if Stateless IPv6 Address Auto Configuration fails");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
      ...
      if (defined $thisKey->{'Attempt_DHCPv6_regardless_of_IPv6_Address_Auto_Config_status'})
      {
         if (lc($thisKey->{'Attempt_DHCPv6_regardless_of_IPv6_Address_Auto_Config_status'}) eq "true")
         {
            $paramName = getGlobalParamName("Host IPv6 Settings", "Attempt DHCPv6 regardless of Stateless IPv6 Address Auto Configuration status");
            if ($paramName ne "")
            {
               if ($postData ne "")
               {
